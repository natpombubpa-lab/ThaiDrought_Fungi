...
```{r}
sample_data(physeq.merge)$plant = factor(sample_data(physeq.merge)$plant, levels = c('Rice', 'Rubber tree', 'Sugarcane', 'Cassava'))
```
...
```{r}
pslayer.zone = plot_ordination(physeq.merge, physeq_ord, type = "samples", color = "plant", shape = "zone") + ggtitle("WHO Fungal Priority Pathogens Beta Diversity (PCoA)") + 
  annotate("text", x = -0.22, y = 0.5,cex = 5.5, label = "PERMANOVA, p = 0.001") + 
...
```{r}
png("./Figures/WHO Fungal Priority Pathogens Beta Diversity(PCoA) 2.png",units = "in", width = 7.5, height = 6, res = 600 )
pslayer.zone + 
  geom_point(size=3)  + 
  theme_pubr(legend = "right", border = TRUE) + 
  theme(plot.title = element_text(hjust = 0.5))
