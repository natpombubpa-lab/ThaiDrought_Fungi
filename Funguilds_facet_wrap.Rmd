...
```{r}
physeq = phyloseq(FGOTU,FGTAX,sampleData)
physeq.two <- subset_samples(physeq, zone == "two")
...
```{r}
sampleData(physeq.prune.nopossible)$zone = factor(sampleData(physeq.prune.nopossible)$zone, levels = c('two','five', 'six'))
```
...
```{r}
#Create a plot
FUNGuildcom = ggplot(data = psmelt(physeq.prune.nopossible), mapping = aes_string(x = "plant" ,y = "Abundance", fill = "trophicMode" )) + geom_bar(stat="identity", position="fill") + ggtitle("Fungal Trophic Mode Composition") + theme_minimal(base_size = 14) + theme(axis.text.x = element_text(angle = 90, hjust = 1)) + scale_fill_manual(values = cbbPalette)  + facet_wrap(~zone)
FUNGuildcom
```


```{r}
png("./Fungal_TrophicMode_plant_8.png", units="in", width = 14, height = 5.5, res = 600 )
FUNGuildcom + theme(legend.position="right") + theme(axis.text.x = element_text(angle = 45, hjust = 1))
dev.off()
```
